  ...
  <ModalPanel @close="cancel">
    ...
    <table class="load-preset-list">
      <tr v-for="(preset, index) in DEFAULT_PRESETS" class="load-preset-item" :key="index">
        ...
        <td><a href="#" class="button small" @click.prevent.stop="loadPreset(preset)">Carregar</a></td>
      </tr>
      <tr v-for="(preset, index) in presets" class="load-preset-item" :key="index">
        <td>{{ index + 1 + NUM_DEFAULT_PRESETS }}</td>
        <td>{{ preset.name }}</td>
        <td><a href="#" class="button small" @click.prevent.stop="loadPreset(preset.data)">Carregar</a></td>
      </tr>
    </table>

    <a href="#" class="button small" @click.prevent.stop="cancel">Cancelar</a>
  </ModalPanel>
</template>
