<template>
  <div class="main-panel wheel-panel">
    <div class="show-options-container">
      <a href="#" class="button" @click.prevent.stop="displayOptions">Opções</a>
    </div>

    <div class="show-records-container">
      <a href="#" class="button" @click.prevent.stop="displayRecords">Ganhos</a>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      spinning: false,
      spinText: 'Girar!',
      resultText: 'Pronto para sortear.'
    };
  },
  computed: mapState({
    spins: state => state.spins,
    name: state => state.data.name || 'Roleta de desconto',
    winningText: state => state.data.winningText || 'Resultado: <b>%s</b>',
